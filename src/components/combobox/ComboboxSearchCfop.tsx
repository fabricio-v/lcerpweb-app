...
import { requestCfopByFilter } from "@/services/requests/cfop";
...
interface Props {
  label?: string;
  valueSelected: ICfopResponse | undefined;
  messageWhenNotfound?: string;
  onChangeValueSelected: (cfop: ICfopResponse) => void;
  disabled?: boolean;
}

export const ComboboxSearchCfop: React.FC<Props> = ({
  label,
  ...
  const searchCfop = async (search: string) => {
    try {
      ...
      const response = await requestCfopByFilter(token!.toString(), search);
