...
interface Props {
  label?: string;
  valueSelected: ICidadeResponse | undefined;
  messageWhenNotfound?: string;
  idEstado: number | null;
  onChangeValueSelected: (cfop: ICidadeResponse) => void;
  disabled?: boolean;
}

export const ComboboxSearchCidade: React.FC<Props> = ({
  label,
  ...
  const { loadCidades, dataCidades, loading } = useSearchCidades();
  const debouncedSearchTerm = useDebouncedValue(searchValue);

  useEffect(() => {
    loadCidades(idEstado, debouncedSearchTerm);
  }, [idEstado, debouncedSearchTerm]);
            ...
            {idEstado === 0 || idEstado === null ? (
              ...
              <>
                ...
                <CommandList>
                  ...
                  {dataCidades.cidades.length === 0 &&
                    searchValue.length > 0 &&
                    !loading && (
                      <CommandEmpty>{messageWhenNotfound}</CommandEmpty>
                    )}
                  <CommandGroup>
                    {dataCidades.cidades.map((item) => (
                      <CommandItem
                        key={item.id}
